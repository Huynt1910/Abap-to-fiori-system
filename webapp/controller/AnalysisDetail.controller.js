    ...
    return BaseController.extend(
      "abap.to.fiori.system.controller.AnalysisDetail",
      {
        ...
        onInit: function () {
          ...
          this.getRouter()
            .getRoute("analysisDetail")
            .attachPatternMatched(this._onRouteMatched, this);
          this.getRouter()
            .getRoute("detail")
            .attachPatternMatched(this._onRouteMatched, this);
          this._oChatBox = new AnalysisChatBox(this);
          ["uiFilters", "databaseObjects", "businessLogic"].forEach(
            function (sSection) {
        ...
        onExit: function () {
          this._iFioriUiRequestVersion =
            (this._iFioriUiRequestVersion || 0) + 1;
        ...
        onRequestHistoryPress: function (oEvent) {
          ...
          if (!oRow || !oRow.analysisId || !oRow.requestId) {
            return;
          }
          if (
            !RequestHistory.sameId(
              oRow.analysisId,
        ...
        _onAnyRouteMatched: function (oEvent) {
          ...
          if (sName !== "analysisDetail" && sName !== "detail") {
            ...
            this._cancelODataGenerationPolling();
            var oGenerationDialog =
              this.byId && this.byId("odataGenerationDialog");
            if (
              oGenerationDialog &&
              oGenerationDialog.isOpen &&
              oGenerationDialog.isOpen()
            ) {
        ...
        _stopRequestHistory: function () {
          ...
          this._iRequestHistoryVersion =
            (this._iRequestHistoryVersion || 0) + 1;
          this._iRequestHistoryDetailVersion =
            (this._iRequestHistoryDetailVersion || 0) + 1;
        ...
        _updateRequestHistoryItems: function () {
          ...
          var aRows = RequestHistory.merge(
            this._aRequestHistoryServerRows || [],
            this._aRequestHistoryOptimisticRows || [],
          );
          this._oViewModel.setProperty(
            "/requestHistory/items",
            RequestHistory.visible(
              aRows,
              this._oViewModel.getProperty("/analysisId"),
              oState.scope,
            ),
        ...
        _scheduleRequestHistoryPoll: function () {
          this._clearRequestHistoryTimer();
          if (
            !this._bRequestHistoryActive ||
            this._pRequestHistoryLoad ||
            !(this._oViewModel.getProperty("/requestHistory/items") || []).some(
              function (oRow) {
          ...
          this._iRequestHistoryTimer = setTimeout(
            function () {
              this._iRequestHistoryTimer = null;
              this._loadRequestHistory();
            }.bind(this),
        ...
        _loadRequestHistory: function () {
          if (!this._bRequestHistoryActive) {
            return Promise.resolve();
          }
          if (this._pRequestHistoryLoad) {
          ...
          this._pRequestHistoryLoad = Promise.resolve()
            .then(
              function () {
            ...
            .then(
              function (oResult) {
                if (
                  !this._bRequestHistoryActive ||
                  iVersion !== this._iRequestHistoryVersion
                ) {
                  return;
                }
                var aServer = oResult.captures
                  .map(function (oRow) {
                    return RequestHistory.normalize(oRow, "CAPTURE");
                  })
                  .concat(
                    oResult.generations.map(function (oRow) {
                      return RequestHistory.normalize(oRow, "GENERATION");
                    }),
                  );
                var mFound = {};
                aServer.forEach(function (oRow) {
                  mFound[RequestHistory.key(oRow)] = true;
                });
                this._aRequestHistoryOptimisticRows = (
                  this._aRequestHistoryOptimisticRows || []
                ).filter(function (oRow) {
                  return (
                    !mFound[RequestHistory.key(oRow)] &&
                    Date.now() - oRow._addedAt < 600000
                  );
                });
              ...
              function (oError) {
                if (
                  ...
                  this._oViewModel.setProperty(
                    ...
                    sMessage && sMessage !== "Unexpected error."
                      ? sMessage
                      : this.getText("requestHistoryLoadError"),
                  );
                }
              }.bind(this),
            )
        ...
        _recordSubmittedRequest: function (
          sKind,
          sAnalysisId,
          sRequestId,
          oResponse,
        ...
        _openRequestHistoryDetail: function (oRow) {
          ...
          this._iLegacyComparisonVersion =
            (this._iLegacyComparisonVersion || 0) + 1;
          if (oRow.kind === "CAPTURE") {
            ...
            this._oViewModel.setProperty(
              "/comparison/captureStatus",
              oRow.status,
            );
            this._oViewModel.setProperty(
              "/comparison/captureMessage",
              oRow.message,
            );
            this._oViewModel.setProperty(
              "/comparison/captureCount",
              oRow.rowCount,
            );
          ...
          this.getAnalysisService()
            .getODataGeneration(sAnalysisId, sRequestId)
            .then(
              function (oResponse) {
                if (
                  iVersion !== this._iRequestHistoryDetailVersion ||
                  !this._bRequestHistoryActive ||
                  !RequestHistory.sameId(
                    sAnalysisId,
                    this._oViewModel.getProperty("/analysisId"),
                ...
                if (
                  ...
                  throw new Error(
                    "Generation response does not match AnalysisId and RequestId.",
                  );
                }
                var oParsed = this._applyODataGenerationResponse(oResponse);
                if (
                  oParsed.status === "FAILED" ||
                  oParsed.status === "DISPATCH_FAILED" ||
                  oParsed.status === "BLOCKED"
                ) {
                  this._oViewModel.setProperty(
                    "/odataGeneration/error",
                    oParsed.message || this.getText("odataGenerationFailed"),
                  );
                }
              }.bind(this),
            )
            .catch(
              function (oError) {
                if (
                  ...
                  this._showODataGenerationError(
                    oError,
                    "requestHistoryDetailError",
                  );
                }
              }.bind(this),
            )
        ...
        onPrepareFioriUi: function () {
          ...
          var sAnalysisId = this._oViewModel.getProperty("/analysisId");
          var sTargetPackage = String(
            this._oViewModel.getProperty("/fioriUi/targetPackage") || "",
          ).trim();
          var sServiceRootUrl = String(
            this._oViewModel.getProperty("/fioriUi/serviceRootUrl") || "",
          ).trim();
          ...
          if (!sAnalysisId || !sTargetPackage || !sServiceRootUrl) {
            this._oViewModel.setProperty(
              "/fioriUi/error",
              this.getText("fioriUiRequiredInputs"),
            );
            return;
          }
          ...
          this.getAnalysisService()
            ...
            .then(
              function (oResponse) {
                ...
                if (
                  iRequestVersion === this._iFioriUiRequestVersion &&
                  ...
                  this._oViewModel.setProperty(
                    ...
                    Object.assign(
                      {},
                      this._oViewModel.getProperty("/fioriUi"),
                      oResult,
                      {
                        busy: false,
                        error: "",
            ...
            .catch(
              function (oError) {
                if (
                  ...
                  var sMessage =
                    oError && oError.message === "Invalid ConfigJson."
                      ? this.getText("fioriUiInvalidConfigJson")
                      : this.parseError(oError).message;
                  this._oViewModel.setProperty(
                    "/fioriUi/error",
                    sMessage && sMessage !== "Unexpected error."
                      ? sMessage
                      : this.getText("fioriUiPrepareError"),
                  );
                }
              }.bind(this),
            )
        ...
        onOpenODataGenerationDialog: function () {
          ...
          if (!sAnalysisId) {
            this._oViewModel.setProperty(
              "/odataGeneration/error",
              this.getText("odataGenerationAnalysisIdRequired"),
            );
            return;
          }
          this._oViewModel.setProperty("/odataGeneration/error", "");
        ...
        onODataGenerationInputChange: function (oEvent) {
          if (
            ODataGeneration.isGenerationActive(
          ...
          var oBinding =
            oSource.getBinding("value") || oSource.getBinding("selectedKey");
          var sPath = oBinding && oBinding.getPath();
          var sValue = oSource.getValue
            ? oSource.getValue()
            : oSource.getSelectedKey();

          if (sPath) {
            this._oViewModel.setProperty(
              sPath,
              ODataGeneration.normalizeUpper(sValue),
            );
          }
        ...
        onPreflightOData: function () {
          ...
          oParameters = ODataGeneration.normalizeParameters(
            oState,
            ODataGeneration.ZERO_UUID,
          );
          if (
            oState.status === "GENERATED" ||
            oState.status === "FAILED" ||
            oState.status === "BLOCKED"
          ) {
            this._oViewModel.setProperty("/odataGeneration/requestId", "");
            this._oViewModel.setProperty(
              "/odataGeneration/generationSignature",
              "",
            );
          }
          this._oViewModel.setProperty(
          ...
          this._oViewModel.setProperty(
            "/odataGeneration/preflightSignature",
            "",
          );
          this._oViewModel.setProperty("/odataGeneration/canGenerate", false);
          ...
          this._setODataGenerationBusy(true);
          this.getAnalysisService()
            .preflightOData(sAnalysisId, oParameters)
            .then(
              function (oResponse) {
                if (
                  iRequestVersion !== this._iODataGenerationRequestVersion ||
                  sAnalysisId !== this._oViewModel.getProperty("/analysisId")
                ) {
                  return;
                }
                ...
                this._oViewModel.setProperty(
                  "/odataGeneration/preflightSignature",
                  bReady ? ODataGeneration.signature(oState) : "",
                );
                this._oViewModel.setProperty(
                  "/odataGeneration/canGenerate",
                  bReady,
                );
                if (oParsed.status === "BLOCKED") {
                  this._oViewModel.setProperty(
                    "/odataGeneration/error",
                    oParsed.message || this.getText("odataGenerationBlocked"),
                  );
                }
              }.bind(this),
            )
        ...
        onGenerateOData: function () {
          ...
          if (
            !sAnalysisId ||
            sValidationError ||
            !oState.preflightReady ||
            oState.preflightSignature !== sSignature
          ) {
            this._oViewModel.setProperty(
              "/odataGeneration/error",
              !sAnalysisId
                ? this.getText("odataGenerationAnalysisIdRequired")
                : sValidationError ||
                    this.getText("odataGenerationPreflightRequired"),
            );
            return;
          }
          ...
          this._setODataGenerationBusy(true);
          this.getAnalysisService()
            .generateOData(sAnalysisId, oParameters)
            .then(
              function (oResponse) {
                if (
                  iRequestVersion === this._iODataGenerationRequestVersion &&
                  ...
                  this._handleODataGenerationStatus(
                    this._applyODataGenerationResponse(oResponse),
                    true,
                  );
                }
              }.bind(this),
        ...
        onConfirmDetailTableSettings: function () {
          ...
          if (!this._validateDetailSettingsState(oState)) {
            this.showErrorMessage(
              this.getText("personalizationAtLeastOneColumn"),
            );
            return;
          }
          ...
          oComparisonService
            ...
            .then(
              function () {
                this._setComparisonProgress(
                  true,
                  ComparisonConstants.progressState.discoveringRun,
                  this.getText("comparisonDiscoveringRun"),
                );
                return oComparisonService.discoverNewRun(
                  sAnalysisId,
                  aPreviousRunIds,
                  iStartedAt,
                );
            ...
            .then(
              function (oRun) {
                this._setComparisonProgress(
                  true,
                  ComparisonConstants.progressState.running,
                  this.getText("comparisonRunning"),
                );
            ...
            .then(
              function (oRun) {
                ...
                var bFailed = oComparisonService.isFailedStatus(
                  oRun && oRun.RunStatus,
                );
        ...
        onSaveMailJob: function () {
          ...
          this._withMailRequestTimeout(
            this.getMailService().createMailJobWithRecipients({
              job: oJob,
            ...
            .then(
              function (oCreated) {
                var sJobId =
                  oCreated && oCreated.object && oCreated.object.JobId;
            ...
            .catch(this._showMailWizardError.bind(this))
            .finally(
              function () {
                this._oMailViewModel.setProperty("/wizard/busy", false);
              }.bind(this),
        ...
        onFrequencyChange: function () {
          var oJob = this.getMailService().normalizeSchedule(
            this._oMailViewModel.getProperty("/wizard/job") || {},
          );
        ...
        onDownloadExport: function () {
          ...
          var bTechnicalDocument =
            oExport.fileFormat === Constants.technicalDocument.fileFormat;
          var pDownload;
          ...
          if (
            bTechnicalDocument
              ? !this._canGenerateTechnicalDocument()
              : !this._canPrepareSelectedExport()
          ) {
          ...
          if (
            !bTechnicalDocument &&
            !(oExport.availableSections || []).some(function (oSection) {
              return (
                oSection.selected !== false &&
                (oSection.fields || []).some(function (oField) {
          ...
          pDownload = bTechnicalDocument
            ? this.getDocumentService().downloadTechnicalDocument(
                this._oViewModel.getProperty("/analysisId"),
                {
                  fileName: sFileName,
                },
              )
            : this.getDocumentService().downloadSelectedExport(
                this._oViewModel.getProperty("/analysisId"),
                {
          ...
          pDownload
            ...
            .catch(
              function (oError) {
                this.showErrorMessage(
                  (oError && oError.message) || this.getText("exportError"),
                );
              }.bind(this),
            )
            .finally(
        ...
        onSourceObjectPress: function (oEvent) {
          ...
          var oSourceObject =
            oTreeNode && (oTreeNode.originalData || oTreeNode);

          if (oSourceObject) {
            this._sSelectedSourceObjectKey =
              oTreeNode.key || SourceObjectTree.getStableKey(oSourceObject);
        ...
        onRecommendationPress: function (oEvent) {
          ...
          if (oRecommendation) {
            this.getRouter().navTo("recommendationDetail", {
              ...
              recommendationId: encodeURIComponent(
                oRecommendation.RecommendationId,
              ),
            });
          }
        },
        ...
        _resetState: function (sAnalysisId) {
          ...
          this._oViewModel.setData(
            models.createAnalysisDetailModel().getData(),
          );
        ...
        _resetLoadedState: function () {
          Object.keys(this._oViewModel.getProperty("/loaded")).forEach(
            function (sKey) {
          ...
          return this.getAnalysisService()
            ...
            .then(
              function (oAnalysis) {
                ...
                return this._loadAnalysisHistory(
                  oAnalysis && oAnalysis.ProgramName,
                );
              }.bind(this),
            )
            .catch(
        ...
        _loadAnalysisHistory: function (sProgramName) {
          ...
          return this.getAnalysisService()
            .readAnalysisHistory(sProgramName)
            .then(
              function (aRows) {
                this._oViewModel.setProperty(
                  "/history/items",
            ...
            .catch(
              function (oError) {
                this._oViewModel.setProperty(
                  "/errors/history",
                  (oError && oError.message) ||
                    this.getText("loadAnalysisHistoryError"),
                );
              }.bind(this),
            )
            .finally(
        ...
        _getDetailRowContext: function (oEvent) {
          var oControl =
            oEvent && (oEvent.getParameter("listItem") || oEvent.getSource());
          var oContext;

          while (oControl) {
            oContext =
              oControl.getBindingContext &&
              oControl.getBindingContext("detail");
            if (oContext) {
              return oContext;
            }
        ...
        _selectSourceObject: function (oSourceObject) {
          ...
          var iRequestedLength = Math.max(
            Number(oSourceObject && oSourceObject.LineCount) || 0,
            10000,
          );
          var sCacheKey = SourceObjectTree.getStableKey(oSourceObject);
          var aEmbeddedLines =
            oSourceObject &&
            (oSourceObject.SourceLines || oSourceObject._SourceLines);
          var aCachedLines = Array.isArray(aEmbeddedLines)
            ? this._normalizeSourceLines(aEmbeddedLines)
            : this._mSourceCodeCache[sCacheKey];
          var iToken;
          ...
          this.getAnalysisService()
            .getSourceLines(sAnalysisId, sSourceItemId, iRequestedLength)
            .then(
              function (aRows) {
                var aLines;
            ...
            .catch(
              function (oError) {
                ...
                this._oViewModel.setProperty(
                  "/sourceCodeDetail/error",
                  (oError && oError.message) ||
                    this.getText("loadSourceLinesError"),
                );
              }.bind(this),
            )
            .finally(
        ...
        _normalizeSourceLines: function (aRows) {
          return (aRows || [])
            ...
            .sort(function (a, b) {
              return (
                Number((a && a.LineNumber) || 0) -
                Number((b && b.LineNumber) || 0)
              );
            })
            .map(function (oRow) {
              return Object.assign({}, oRow, {
                SourceText:
                  oRow &&
                  oRow.SourceText !== null &&
                  oRow.SourceText !== undefined
                    ? String(oRow.SourceText)
                    : "",
              });
            });
        },
        ...
        _resetSourceCodeDetail: function () {
          this._iSourceCodeRequestToken =
            (this._iSourceCodeRequestToken || 0) + 1;
        ...
        _showSourceCodeSection: function () {
          ...
          if (
            oPage &&
            oSubSection &&
            typeof oPage.scrollToSection === "function"
          ) {
        ...
        _onSourceObjectResize: function (oEvent) {
          var iWidth =
            (oEvent && oEvent.size && oEvent.size.width) || Device.resize.width;
        ...
        _selectBusinessLogic: function (oRow) {
          ...
          this.getAnalysisService()
            .getBusinessLogicCallBindings(sAnalysisId, sItemId)
            .then(
              function (aRows) {
                this._oViewModel.setProperty(
                  "/businessLogicDetail/callBindings",
                  aRows || [],
                );
              }.bind(this),
            )
            .catch(
              function (oError) {
                this._oViewModel.setProperty(
                  "/businessLogicDetail/error",
                  (oError && oError.message) ||
                    this.getText("loadCallBindingsError"),
                );
              }.bind(this),
            )
            .finally(
        ...
        _resetExportState: function (mOptions) {
          var sSelectedSection =
            (mOptions && mOptions.section) ||
            this._oViewModel.getProperty("/selectedTab") ||
            Constants.section.sourceObjects;
          var oConfig = AnalysisTableConfig.getConfig(sSelectedSection);
          var aSections =
            (mOptions && mOptions.sections) ||
            this._buildSectionExportSelection(sSelectedSection);
          var aFields = this._flattenExportSections(aSections);
          var sFileFormat = Constants.fileFormat.excel;
          var sExportSection =
            (mOptions && mOptions.exportSection) ||
            (oConfig && oConfig.exportSection) ||
            "";
          var sReportType =
            this._oViewModel.getProperty("/overview/ProgramName") || "";
          ...
          this._oViewModel.setProperty("/export", {
            ...
            selectedSection:
              (mOptions && mOptions.selectedSection) || sSelectedSection,
            selectedSectionLabel:
              (mOptions && mOptions.selectedSection) ===
              Constants.exportSection.all
                ? ""
                : this._getExportSectionLabel(sSelectedSection),
        ...
        _getExportFileName: function (oExport) {
          ...
          return (
            sFileName ||
            (oExport && oExport.defaultFileName) ||
            this.getDocumentService().getFallbackFileName({
        ...
        _buildExportFieldSelection: function (sSection) {
          ...
          var aVisibleFields = oConfig
            ? this._oTablePersonalization.getVisibleExportableFields(
                oConfig,
                oState,
              )
            : [];
          ...
          aFields = ((oConfig && oConfig.exportFields) || [])
            .map(function (sKey) {
              return mFields[sKey];
            })
            .filter(Boolean);
        ...
        _canPrepareSelectedExport: function () {
          var oControl = this._oViewModel.getProperty(
            "/overview/__OperationControl",
          );
        ...
        _validateODataGeneration: function (oState, bGenerate) {
          ...
          if (
            String((oState && oState.providerLanguage) || "").toUpperCase() ===
              "STANDARD" &&
          ...
          if (
            bGenerate &&
            !String((oState && oState.transportRequest) || "").trim()
          ) {
        ...
        _setODataGenerationInputs: function (oParameters) {
          this._oViewModel.setProperty(
            "/odataGeneration/targetPackage",
            oParameters.TargetPackage,
          );
          this._oViewModel.setProperty(
            "/odataGeneration/providerPackage",
            oParameters.ProviderPackage,
          );
          this._oViewModel.setProperty(
            "/odataGeneration/providerLanguage",
            oParameters.ProviderLanguage,
          );
          this._oViewModel.setProperty(
            "/odataGeneration/transportRequest",
        ...
        _applyODataGenerationResponse: function (oResponse) {
          ...
          var sRequestId = ODataGeneration.isUsableRequestId(oParsed.requestId)
            ? oParsed.requestId
            : this._oViewModel.getProperty("/odataGeneration/requestId");
          ...
          this._oViewModel.setProperty(
            "/odataGeneration/status",
            oParsed.status,
          );
          this._oViewModel.setProperty(
            "/odataGeneration/runtimeCheck",
            oParsed.runtimeCheck,
          );
          this._oViewModel.setProperty(
            "/odataGeneration/message",
            oParsed.message,
          ...
          this._oViewModel.setProperty(
            "/odataGeneration/result",
            oParsed.result,
          );
          this._oViewModel.setProperty(
            "/odataGeneration/resultJsonInvalid",
            oParsed.resultJsonInvalid,
          );
          this._oViewModel.setProperty("/odataGeneration/hasResult", true);
          this._oViewModel.setProperty("/odataGeneration/error", "");
          return oParsed;
        ...
        _handleODataGenerationStatus: function (oParsed, bStartPolling) {
          ...
          if (oParsed.status === "GENERATED") {
            this._oViewModel.setProperty("/odataGeneration/canGenerate", false);
            MessageToast.show(
              oParsed.message || this.getText("odataGenerationGenerated"),
            );
          } else if (
            oParsed.status === "BLOCKED" ||
            oParsed.status === "FAILED"
            ...
            this._oViewModel.setProperty(
              ...
              oParsed.message ||
                this.getText(
                  oParsed.status === "BLOCKED"
                    ? "odataGenerationBlocked"
                    : "odataGenerationFailed",
                ),
            );
          }
        },
        ...
        _scheduleODataGenerationPoll: function () {
          this._iODataGenerationTimer = setTimeout(
            function () {
        ...
        _pollODataGeneration: function (bManual) {
          var sAnalysisId = this._oViewModel.getProperty("/analysisId");
          var sRequestId = this._oViewModel.getProperty(
            "/odataGeneration/requestId",
          );
          var iRequestVersion;
          ...
          if (
            !sAnalysisId ||
            !ODataGeneration.isUsableRequestId(sRequestId) ||
            this._oViewModel.getProperty("/odataGeneration/dialogBusy")
          ) {
            if (!ODataGeneration.isUsableRequestId(sRequestId)) {
              this._oViewModel.setProperty(
                "/odataGeneration/error",
                this.getText("odataGenerationRequestIdRequired"),
              );
            }
            return;
          }
          ...
          this.getAnalysisService()
            ...
            .then(
              function (oResponse) {
                ...
                if (
                  iRequestVersion !==
                    (this._iODataGenerationRequestVersion || 0) ||
                  sAnalysisId !== this._oViewModel.getProperty("/analysisId")
                ) {
                  return;
                }
                ...
                if (
                  !bManual &&
                  ODataGeneration.isGenerationActive(oParsed.status)
                ) {
                  this._iODataGenerationPollCount =
                    (this._iODataGenerationPollCount || 0) + 1;
                  if (this._iODataGenerationPollCount >= 12) {
                    this._oViewModel.setProperty(
                      "/odataGeneration/polling",
                      false,
                    );
                    this._oViewModel.setProperty(
                      "/odataGeneration/message",
                      oParsed.status === "QUEUED"
                        ? this.getText("odataGenerationWorkerHint")
                        : this.getText("odataGenerationPollingStopped"),
                    );
                  } else {
                    this._scheduleODataGenerationPoll();
                  }
        ...
        _cancelODataGenerationPolling: function () {
          this._clearODataGenerationTimer();
          this._iODataGenerationRequestVersion =
            (this._iODataGenerationRequestVersion || 0) + 1;
          if (this._oViewModel) {
            this._oViewModel.setProperty("/odataGeneration/polling", false);
        ...
        _applyMailFrequencyUiState: function (sFrequency) {
          this._oMailViewModel.setProperty(
            "/wizard/schedule",
            this.getMailService().getFrequencyUiState(sFrequency),
          );
        },
        ...
        _getSectionKeyFromId: function (sSectionId) {
          ...
          mSectionIds[Constants.section.sourceObjects] =
