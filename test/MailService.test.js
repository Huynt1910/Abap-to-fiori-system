...
const MailConstants = loadUi5Module(
  path.join(root, "webapp", "model", "mailConstants.js"),
);
const ODataErrorHandler = loadUi5Module(
  path.join(root, "webapp", "util", "ODataErrorHandler.js"),
);
const mailFormatter = loadUi5Module(
  path.join(root, "webapp", "model", "mailFormatter.js"),
);
...
class Filter {
  constructor(pathOrConfig, operator, value) {
    Object.assign(
      this,
      typeof pathOrConfig === "object"
        ? pathOrConfig
        : { path: pathOrConfig, operator, value },
    );
  }
}
...
const MailService = loadUi5Module(
  path.join(root, "webapp", "service", "MailService.js"),
  {
...
test("mail formatter maps job, execution, frequency and recipient codes", () => {
  assert.equal(
    mailFormatter.formatExecutionStatus("S"),
    "Accepted by SAPconnect",
  );
  assert.equal(mailFormatter.formatExecutionStatusState("F"), "Error");
  assert.match(
    mailFormatter.formatExecutionStatusTooltip("S"),
    /Final SMTP delivery status/,
  );
...
test("OData error parser handles 412 and missing SU01 email", () => {
  assert.equal(
    ODataErrorHandler.parse({ status: 412, message: "Precondition Failed" })
      .message,
...
test("schedule validation requires dynamic fields for scheduled jobs", () => {
  ...
  assert.match(
    service
      .validateSchedule({ Frequency: "D", StartDate: tomorrow }, true)
      .join("\n"),
    /Start time/,
  );
  assert.match(
    service
      .validateSchedule(
        { Frequency: "D", StartDate: tomorrow, StartTime: "08:00:00" },
        true,
      )
      .join("\n"),
    /Time zone/,
  ...
  assert.match(
    service
      .validateSchedule(
        { Frequency: "D", StartDate: "08/10/2026", StartTime: "08:00:00" },
        true,
      )
      .join("\n"),
    /YYYY-MM-DD/,
  ...
  assert.match(
    service
      .validateSchedule(
        { Frequency: "D", StartDate: tomorrow, StartTime: "8 AM" },
        true,
      )
      .join("\n"),
    /HH:mm:ss/,
...
test("schedule payload resets non-applicable fields and keeps month day unpadded", () => {
  ...
  assert.deepEqual(
    JSON.parse(JSON.stringify(service.getFrequencyUiState("W"))),
    {
      showStartDate: true,
      showStartTime: true,
      showDayOfWeek: true,
...
test("create job runs Inactive to Recipients to Active and omits EmailAddress", async () => {
  ...
  const jobContext = {
    ...
    setProperty: async (property, value) =>
      calls.push(["patch", property, value]),
      ...
      if (pathValue === "/MailJobs") {
        return {
          create(payload) {
            ...
            assert.equal(
              payload.StartTime,
              MailConstants.scheduleDefaults.startTime,
            );
            assert.equal(
              payload.JobTimeZone,
              MailConstants.scheduleDefaults.jobTimeZone,
            );
            assert.equal(
              payload.DayOfWeek,
              MailConstants.scheduleDefaults.dayOfWeek,
            );
    ...
    recipients: [
      {
        RecipientType: "T",
        SapUser: "DEV-130",
        EmailAddress: "ignore@example.com",
      },
    ],
  ...
  assert.deepEqual(
    ...
    ["createJob", "createRecipient", "patch"],
  );
...
test("update job does not patch AnalysisId or Status from the edit wizard copy", async () => {
  ...
  assert.equal(
    calls.some((call) => call[0] === "AnalysisId"),
    false,
  );
  assert.equal(
    calls.some((call) => call[0] === "Status"),
    false,
  );
  assert.equal(
    calls.some((call) => call[0] === "JobName"),
    true,
  );
});
...
test("recipient create rejects immediately when createCompleted reports backend failure", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.equal(
        ODataErrorHandler.parse(error).message,
        "SAP user SA does not exist",
      );
      return true;
    },
  );
