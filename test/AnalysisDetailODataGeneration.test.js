...
test("failed OData responses show the backend message once as an error", () => {
  const controller = createController();
  const backendMessage = "Report logic must be extracted into a reusable provider class.";

  ["BLOCKED", "FAILED", "DISPATCH_FAILED"].forEach((status) => {
  ...
  assert.equal(controller._oViewModel.getProperty("/odataGeneration/message"), "Ready to generate");
});
